...
public class RequirementPackageHelper implements IHelper {
	...
	public Object getValue(EObject object, EStructuralFeature feature, EAnnotation annotation) {
		...
		if (ret == null && object instanceof SystemFunctionalInterfaceRequirement) {
			ret = SystemFunctionalInterfaceRequirementHelper.getInstance()
					.doSwitch((SystemFunctionalInterfaceRequirement) object, feature);
		}
		if (ret == null && object instanceof SystemFunctionalRequirement) {
			ret = SystemFunctionalRequirementHelper.getInstance().doSwitch((SystemFunctionalRequirement) object,
					feature);
		}
		if (ret == null && object instanceof SystemNonFunctionalInterfaceRequirement) {
			ret = SystemNonFunctionalInterfaceRequirementHelper.getInstance()
					.doSwitch((SystemNonFunctionalInterfaceRequirement) object, feature);
		}
		if (ret == null && object instanceof SystemNonFunctionalRequirement) {
			ret = SystemNonFunctionalRequirementHelper.getInstance().doSwitch((SystemNonFunctionalRequirement) object,
					feature);
		}
