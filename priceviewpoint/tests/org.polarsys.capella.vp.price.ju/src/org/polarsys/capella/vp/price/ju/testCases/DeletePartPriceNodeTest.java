...
/**
 * This test case checks if when deleting a partPrice of a node PC the price of
 * its parents is re-calculated.
 * Used to check if the listener responds to the notification REMOVE
 */
public class DeletePartPriceNodeTest extends PriceTest {

	private PartPriceImpl pc1PartPrice;
	private PartPriceImpl pc11PartPrice;
	...
	public void test() throws Exception {
		...
		ExecutionManager manager = TransactionHelper.getExecutionManager(pc11PartPrice);
		manager.execute(new AbstractReadWriteCommand() {

			@Override
			public void run() {
				EObjectHelper.removeElement(pc11PartPrice);
			}

		});
		assertEquals("The price of PC1 was not changed after the deletion of the part price of PC 1.1",
